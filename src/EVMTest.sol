 ...
 *
 * This is a contract generated at 2022-11-05 18:30:27 for making requests to and receiving responses from midpoint 425. 
...
contract SampleContract {
    ...
    event RequestMade(uint256 requestId, string ach);
    event ResponseReceived(uint256 requestId, string ach, string topLevelHash);
    event Confirmed();
    event Denied();
    ...
    // Midpoint ID
    uint64 constant midpointID = 425;
     ...
     * 
     * Note that this is a public function and will allow any address or contract to call midpoint 425.
    ...
    function callMidpoint(string memory ach) public {
        
        // Argument String
        bytes memory args = abi.encodePacked(ach, bytes1(0x00));
        
        // Call Your Midpoint
        uint256 requestId = IMidpoint(startpointAddress).callMidpoint(midpointID, args);

        // For Demonstration Purposes Only
        emit RequestMade(requestId, ach);
    }
   ...
   function callback(uint256 _requestId, uint64 _midpointId, string memory ach, string memory topLevelHash) public {
       ...
       // For Demonstration Purposes Only
       emit ResponseReceived(_requestId, ach, topLevelHash);
   }

   function checkValidAch(string memory _ach) public view returns(bool) {
   ...
   function postResponse(uint256 _validatorKey) public {
        affirms[responses] = _validatorKey;
        responses += 1;
   }
}
